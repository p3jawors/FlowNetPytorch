...
def get_kitti_vo(vo_dir):
    # vo from flownetsimple will be 311x94x2=58468 per scene
    files = os.listdir(vo_dir)
    print('all: ', files[:5])
    files[:] = [name for name in files if any(sub in name for sub in ['.npy'])]
    print('npy only: ', files[:5])
    files = sorted(files)
    print('sorted!: ', files[:5])
    for name in files:
        flow = np.load('%s/%s' % (vo_dir, name))
        print('flow: ', flow.shape)
    # img_pairs = []
    # for ii in range(0, len(files)-1):
    #     img1_file = '%s/%s' % (args.data, files[ii])
    #     img2_file = '%s/%s' % (args.data, files[ii+1])
...
def main(vo_dir, batch_size=100, n_epochs=10, logdir='ps_mnist_lmu', seed=13, gpu=-1):
    ...
    X_train, X_valid, X_test, Y_train, Y_valid, Y_test = get_kitti_vo(vo_dir)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        'Train a Legendre Memory Unit model to predict visual odometry from optical flow'
    )
    ...
    parser.add_argument('--vo_dir', type=str, default=(
        '/media/pjaworsk/5686139C86137C25/tb_storage/datasets/KITTI/data_odometry_color/dataset/sequences/00/image_2/flow'),
                        help="Path to kitti sequences/scene_num/image_2/flow folder")
    args = parser.parse_args()
    main(vo_dir=args.vo_dir, batch_size=args.batch_size, n_epochs=args.n_epochs, logdir=args.logdir, seed=args.seed, gpu=args.gpu)
